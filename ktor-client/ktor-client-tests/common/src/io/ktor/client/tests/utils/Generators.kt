...
suspend fun List<PartData>.makeString(): String = buildString {
    ...
    list.forEach {
        append("${it.name!!}\n")
        val content = when (it) {
            is PartData.FileItem -> filenameContentTypeAndContentString(it.provider().readString(), it.headers)
            is PartData.FormItem -> it.value
            is PartData.BinaryItem -> filenameContentTypeAndContentString(it.provider().readString(), it.headers)
...
private suspend fun filenameContentTypeAndContentString(content: String, headers: Headers): String {
